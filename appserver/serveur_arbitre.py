...
def receive_movement(action: MovementAction, robot_id: str = "marty_01") -> MovementResponse:
    signals.new_log.emit(f"Mouvement reçu de {robot_id} :\n{json.dumps(action.model_dump(), indent=4, ensure_ascii=False)}")
    with robots_lock:
        ...
        if robot_id not in robot_threads:
            thread = RobotThread(robot_id, robot_sessions[robot_id].team)
            thread.start()
            robot_threads[robot_id] = thread
            robots_scores[robot_id] = robot_sessions[robot_id].current_score
        thread = robot_threads[robot_id]
...
def robot_connect(session: RobotSession):
    with robots_lock:
        robot_sessions[session.robot_id] = session
...
def robot_disconnect(session: RobotSession):
    with robots_lock:
        ...
        robot_sessions.pop(session.robot_id, None)
    if thread:
        thread.stop()
